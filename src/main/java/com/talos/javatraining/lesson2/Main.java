...
//		result.setHumidity(makeItReadable(measure.getHumidity(), "%"));
		...
		newMeth(measure::getWindSpeed,result::setWindSpeed, "mph");
		newMeth(measure::getTemperature,result::setTemperature, "°C");
		newMeth(measure::getPressure,result::setPressure, "pa");
		newMeth(measure::getHumidity,result::setHumidity, "%");

		return result;
	}

	protected  void newMeth(Supplier<BigDecimal> getter, Consumer <String> result, String unit){
		result.accept(makeItReadable(getter.get(),unit));
	}
